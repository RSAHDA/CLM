...
INSTALLED_PATH = "C:/Users/Yaten/PycharmProjects/clm/CLM/v1/"
...
def installCmakeBuild(location):
    if not exists(location):
        ...
        exit(0)
    elif not exists(f"{location}/CMakeLists.txt"):
        print(f"[*] ERROR, {location} is not eligible of Cmake build.")
        exit(0)
    ...
    shutil.rmtree(f"{INSTALLED_PATH}/builds")
    shutil.rmtree(f"{INSTALLED_PATH}/final")
...
def installWithoutCmakeBuild(location):
    if not exists(location):
        print(f"[*] {location}, doesnt exists.")
        exit(0)
    elif not (exists(f"{location}/include") and exists(f"{location}/lib")):
        print(f"[*] ERROR, {location} not eligible for installation.")
        exit(0)
    print(f"[*] INSTALLING {location}")
    try:
        os.system(f'robocopy {location}/include C:/MinGW/include /e /ndl & robocopy {location}/lib C:/MinGW/lib /e /ndl')
    except:
        print("[*] ERROR, library not eligible for download.")
